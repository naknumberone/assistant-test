...
И в `POST /api/chat`, и при `loadChat` из файлового хранилища история дополнительно прогоняется через `safeValidateUIMessages`. Если валидация не проходит, код не бросает ошибку, а подменяет историю на `[]`: битый чат для UI выглядит пустым.
